...
6. **LED Matrix Flicker**: The micro:bit LED matrix is multiplexed. If you call `display.clear()` inside a rapid `while True:` loop before redrawing your graphics, the LEDs will physically turn off for a microsecond every loop, creating a harsh strobing or flickering effect. Instead of clearing the display, explicitly overwrite the pixel values (e.g., setting them to `0` or `9` directly) so lit LEDs remain continuously powered.
6. **Varying Transmit Power vs RSSI**: Because the Fox cycles through different transmit power levels (Max, Med, Low) to create the audio zones, the Hound will receive those packets with artificially fluctuating RSSI values even if it is stationary. To prevent the bar graph from constantly bouncing up and down in sync with the Fox's transmit cycle, the Hound must **only** update its distance/RSSI calculation when receiving the `Z1` (Max Power) beacon, while using the `Z2`/`Z3` beacons purely for audio zone logic.
