...
1. **The Fox (Transmitter)**: a hidden beacon -- `fox` in the code, the **Treasure** in everything a user reads (section 5). To create proximity zones without asking the seeker to interpret raw radio data, it cycles its transmit power (max, medium, low) once every 200 ms. It is dark apart from one deliberate exception: either button shows its radio group for about a second.
...
So when you need a reset you can rely on, ask for one: Ctrl-C to the REPL and type `import microbit` / `microbit.reset()`. That is a real hardware reset, which section 7's gotcha 10 shows is the clean one, and it is deterministic — 4 of 4 where opening the port was 0 of 1. `flash.py`'s `_hard_reset` does exactly this, and everything up to the echo of the command is host chatter to be discarded, not board output.
...
**"Flasher" is avoided in anything a user reads.** It is an unfortunate word on a page aimed at schools and Scout groups, and the site is about the *activity* -- setting the boards up is one part of it, not the point of it. So the page is "Radio Treasure Hunt", buttons say "Set up as Treasure", and the footer stamp is labelled "Site". Code identifiers keep it (`flasher.js`, the `Flasher` class, `flash.py`, `doFlash`), because "flash" is the correct technical verb for writing firmware and renaming them would churn code nobody reads. A test asserts the word appears in no user-facing page.

**Neither is "fox hunt", as the name of the activity.** Renamed 2026-09-07: to a child who has not met the hobby, "fox hunt" is the blood sport, and a school or Scout group should not have to explain that before anyone has switched a board on. Every page a user reads is the **Radio Treasure Hunt** -- title, headings, nav links and footer stamps. The hobby's own name is kept as a single aside in the introduction, next to "amateur radio direction finding", because it is what someone would search for and it is worth a child knowing; `test_no_user_facing_page_is_titled_a_fox_hunt` pins both halves of that. **The Hound stays. The fox became the Treasure** (2026-09-07, second pass): a hound sniffing out a signal is a fair picture of what the receiver does, but a hound chasing a fox is the sport again, and the hidden board is the thing you are hunting for -- so to a reader it is the **Treasure**. `test_the_hidden_board_is_the_treasure_to_a_reader` strips attribute values, allows the "fox hunt" aside, and fails on any other Fox in the four pages. **The fox emoji is banned outright** -- U+1F98A, literal or numeric -- and the pages use U+1F48E GEM STONE instead; that has its own test, because a stray fox on a page that never says fox is exactly the sort of thing nobody sees by eye.

**The seam is `ROLE_LABELS` in `build_site.py`.** The role *key* is `fox` everywhere it is machinery -- `flash.ROLES`, `fox.py`, `id="flash-fox"`, `class="fox"`, the manifest -- and `ROLE_LABELS["fox"] = "Treasure"` is the one place the word a user reads is decided. It reaches the flashing UI, the identify result, the session table and the per-group tally through the manifest, so those cannot drift from the pages; a test pins the label, and the `.hex` download filename maps the key through the same word rather than shipping `fox` into someone's Downloads folder. Code identifiers otherwise keep `foxhunt` throughout -- the repo directory, `pyproject.toml`, `GROUP_KEY`, the service-worker cache, `--FOXHUNT--`.
...
**One board operation at a time, and `withBoard()` is the only guard.** The page drives a single USB connection, so two operations must never overlap -- a flash interleaved with another flash, or with the disconnect behind "Done with this board". That guard used to be a button list copied into each operation and it had already drifted (2026-09-08): `doFlash` disabled four controls, `connect` only itself, and `startMonitor`, `nextBoard` and the forget handler none at all. `withBoard()` now holds the single copy of the control list, disables all of them, refuses re-entry regardless -- a disabled button is not proof, because the second click of a double-click can land before the first has disabled anything, and `#next-board` is rendered into a result banner rather than existing up front -- and restores each button to what it *was* rather than blanket-enabling, so a state set for another reason (no WebUSB, an unusable group) survives. Every route to the board goes through it and nothing guards itself; the test pins both halves.

**Every route that writes a board shows the same progress bar.** Setting a board up as a role and turning it into the radio monitor are the same 23-second wait, and until 2026-09-08 only the first had a bar. `progressCard()` renders the markup into whichever container is being watched rather than the page carrying two copies of it: the monitor's has to live in its own panel, because starting it scrolls that panel to the top of the screen and leaves the card in "Set it up" just above the viewport.

**The page is fetched network first; everything else is cache first.** The bundle is named by build stamp -- `app.<short sha>.js` -- so every deploy removes the file a cached page names, and the service worker's `activate` deletes the cache that held it. Cache-first navigation therefore served a page whose script 404s: no JavaScript at all, every control dead, and the radio group box empty. It reads as a page still loading rather than one that has failed, which is why it was reported as "the radio group is unset". Reproduced 2026-09-08 by putting a pre-deploy `index.html` into the live cache -- one console 404, empty box, connect disabled -- and the fix verified the same way, with a doctored entry naming a bundle that does not exist: the page came from the network with the right bundle. Assets stay cache first, which is what makes the site work in a field.

**The page works offline; flashing offline needs someone to have flashed once first.** Measured 2026-09-08 with the local server stopped: the page, the bundle and all five device sources came out of the cache, `loadManifest()` completed and the group note rendered. But `install` precaches only the page, the manifest and the stylesheet; everything else is cached as it is fetched, and the 1.2 MB MicroPython image is fetched only by the first flash. So an organiser who loads the page at home and flashes nothing arrives in the field with no image to flash with. `sw.js` claimed the opposite in its own header until this was checked. This is the first half of section 8's offline item; it is still not a substitute for a real disconnected machine.

**The radio group box carries its default in the HTML.** It was filled only by JavaScript, so it sat blank for as long as the manifest and the device sources took to arrive -- and blank forever when the script did not run. An empty box labelled "Radio group" reads as "you have not chosen yet". It is stamped through the existing `__GROUP_DEFAULT__` substitution, so it cannot drift from `RADIO_GROUP`.

**The site counts how it is used, and the rules for that are not decoration.** `web/src/analytics.js` sends page views and a closed set of event names to a self-hosted GoatCounter at `https://murdoch.is/stats/count`, all of it under the `projects/radio-treasure-hunt/` namespace -- our own software on our own box (`wordpress-prod`, deployed by `site-wordpress.yml` in the sysadmin repo), never a third party. The audience is children, so: no cookies, no `localStorage`, no identifier of any kind, nothing sold or shared, and a "What this page counts" note on the page that a test holds to what the code can actually send. Neither Do Not Track nor Global Privacy Control is checked -- DNT is a withdrawn standard, and GPC is a signal about selling and sharing data, which this site does not do. Four things are load-bearing:

* **The event vocabulary is a closed set**, built in `build_site.py` from `flash.ROLES`. The page holds a board's DAPLink serial number; an allowlist is what stops a later edit beaconing it, and `test_every_event_name_the_javascript_sends_is_in_the_allowlist` scans the call sites. Role-shaped names are built by helpers in `analytics.js` for exactly that reason -- they are the only non-literal ones, so the scan can still see them.
* **Off unless configured, and configured in one place.** `[tool.foxhunt.analytics]` in `pyproject.toml`, overridable by `$FOXHUNT_ANALYTICS_*` and then by `--analytics-endpoint` / `--analytics-hosts` / `--no-analytics`. Empty endpoint means no beacon, which is what a fork and a local `make site` get; `hosts` is a second guard, so a fork that keeps the committed config still sends nothing because it is served from another host. `build_site.py` prints the endpoint it built with, so a build that lost its analytics shows up in the CI log.
* **Everything reported shares one namespace.** The hunt is counted inside `murdoch.is`'s own GoatCounter site, because it is part of that personal site rather than a thing of its own -- so `projects/radio-treasure-hunt/` in front of every path and event is what keeps it identifiable -- inside that site's real `/projects/` section, the hunt being one project among others, and one dashboard filter separates the two in both directions. Verified against real GoatCounter on the VM, 2026-09-08. It is applied in `analytics.js`'s `event()` and `pagePath()`, once, so the allowlist and the call sites stay bare.
* **The group number is never sent** -- `group/default` or `group/other`. Whether organisers move off 16 is the open question; the value answers nothing more.
* **Impact numbers are proxies and the plan says so.** `hunt/ready` (a Treasure and a Hound set up in one sitting) and `session/boards/10+` are the numbers worth reporting, but a prepared hunt is not a played one, blockers and offline use mean every figure is a floor, and a session is a page load rather than a person. `archive/ANALYTICS_PLAN.md` section 5 is the reference before any of this goes in a report -- the one part of that file that is still guidance rather than context.

**The bundles are built for browsers that cannot flash.** esbuild now targets `es2019`. The bundle carried optional chaining (ES2020), so Safari 13.0 and older Firefox failed to *parse* it -- not a degraded page but no JavaScript at all: dead controls, an empty group box, no `.hex` download and no analytics, on precisely the browsers whose only route is that download. WebUSB stays a runtime feature check, and `webusb/available` / `webusb/unsupported` measure how large that population is rather than guessing it from a user agent. `test_the_bundles_parse_on_browsers_that_cannot_flash` builds both bundles for real and fails on either ES2020 operator.

**The teaching pages are bundled now, not copied.** `web/teaching.js` became `web/src/teaching.js` and is a second esbuild entry point, stamped like the main one, because it shares `analytics.js` with the setup page -- a copied script would have been a second copy of the beacon.
...
5. **Ctrl-D inside the raw REPL means "execute"**, and is unrelated to Ctrl-D at the friendly prompt, which is the heap-corrupting soft reboot in section 7's gotcha 10. Likewise `softwareReset()` in microbit-connection is an SWD reset -- the clean kind. Both names are alarming and both are safe.
...
10. **The radio monitor drains the serial buffer that `bootCheck` and `verify` read from.** `SerialLink.buf` is one shared accumulator, and the monitor's loop takes it and empties it every 150 ms -- so an operation started underneath a running monitor reads a buffer someone else is emptying. Both outcomes are wrong and one is dangerous: `verify` waits for `>>>` that it may never see, and fails a board that is fine; `bootCheck` treats an empty capture as success, because the fox and the hound print nothing when healthy, so a board that raised a `Traceback` is reported as *set up and running*. This is section 9's "never conclude anything from a board's silence" arriving through the buffer rather than the board. `withBoard()` stops the monitor at entry, which is why nothing else does. Read out of the code on 2026-09-08, not seen on a board.
...
**The counting, now that it is deployed**

* **A page view from the deployed site, in a real browser.** The end-to-end check on 2026-09-08 sent beacons built by `analytics.js` itself to the live endpoint and confirmed they stored in the `murdoch.is` site under the namespace -- so the wire format and the server are settled. What it skipped is the browser: a page load from `sjmurdoch.github.io` producing a hit on the dashboard on its own, through the deployed bundle and the `hosts` guard. The deployed bundle does carry both (measured 2026-09-08 by fetching `app.c0c7162.js` from the live site), so this is watching one fire, not building anything.
* **The two server-side checks**, from section 9 of the archived plan and not recorded as done: that GoatCounter is not retaining IP addresses beyond what its session hash needs, and that the dashboard is not publicly readable unless that is wanted. Neither can be settled from this repo.
* **Whether a school network or a managed Chromebook blocks the stats host.** Plausible, and it would bias exactly the population most worth measuring -- so every figure is a floor by an unknown amount. Section 5 of the archived plan is explicit about what that does and does not license anyone to claim.
...
* `ANALYTICS_PLAN.md` — the plan the site's usage counting was built from, archived 2026-09-08. All of it is built and deployed; the design is in section 5 above and what remains is carried in section 8. Its own section 9 is superseded by its section 2 and says so. Read it for how the hosting and privacy decisions were reached -- and for its section 5, which is still the reference before quoting any of these numbers.
