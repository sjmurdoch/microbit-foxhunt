...
* **Everything reported shares one namespace.** The hunt is counted inside `murdoch.is`'s own GoatCounter site, because it is part of that personal site rather than a thing of its own -- so `radio-treasure-hunt/` in front of every path and event is what keeps it identifiable, and one dashboard filter separates the two in both directions. Verified against real GoatCounter on the VM, 2026-09-08. It is applied in `analytics.js`'s `event()` and `pagePath()`, once, so the allowlist and the call sites stay bare.
