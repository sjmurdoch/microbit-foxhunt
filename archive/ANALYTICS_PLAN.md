...
Status: built and archived 2026-09-08. Written 2026-09-08 and implemented the same day; the collector is deployed and the site half is live. What follows is the reasoning as it stood while it was built, not current guidance -- `AGENTS.md` section 5 carries the design and section 8 tracks what is still open. **Section 5 below is the exception**: it is what the numbers do and do not license anyone to claim, and `AGENTS.md` points at it as the thing to read before any of this goes in a report.
...
**The hunt is counted as part of murdoch.is, not as a site of its own.** GoatCounter is multi-site and selects a site by the `Host` header, so a separate site was possible -- but the hunt is part of the owner's personal site rather than a thing standing on its own, and it is counted that way. What that costs is separability, and a namespace buys it back: **every path this site reports, page views and events alike, goes under `projects/radio-treasure-hunt/`**, so one filter on the dashboard shows the hunt and only the hunt, and the personal site's own paths stay clean of it. It sits inside that site's real `/projects/` section, the hunt being one project among others: `projects/` then reads as all of them and `projects/radio-treasure-hunt` as this one. Checked 2026-09-08 -- `https://murdoch.is/projects/` is a real page and `https://murdoch.is/projects/radio-treasure-hunt` is a 404, so nothing already served collides with what is reported.
...
**Superseded by section 2, and left here only to show what it replaced.** The sketch below was written before the hosting decision, when a separate `stats.` name still looked likely: it stands GoatCounter up on its own hostname with its own TLS, which is not what was built.
...
./goatcounter serve -listen=:443 -tls=tls,rdr,acme     # superseded: see section 2
```

What was actually deployed is section 2's arrangement -- no new name, GoatCounter on the loopback under `-base-path /stats` with `-vhost=murdoch.is`, Apache reverse-proxying `/stats` on the existing vhost with `ProxyPreserveHost On`, so the endpoint is `https://murdoch.is/stats/count`. The deployment itself lives in the sysadmin repo (`site-wordpress.yml` on `wordpress-prod`), not here. It is up: hits generated by the module itself were confirmed stored in the `murdoch.is` site on 2026-09-08, with the namespace separating them from the site's own paths.

Two things to check on the server, neither of which can be checked from here and neither of which is recorded as done:
...
Both are carried in `AGENTS.md` section 8, which is where they are tracked now.

* End-to-end is **half verified**. 2026-09-08: beacons built by the module itself were sent to the live endpoint and stored in the right site under the right namespace, so the wire format and the server are settled. What that route skipped is the browser -- a real page load from `sjmurdoch.github.io`, through the deployed bundle and the `hosts` guard, producing a hit on the dashboard by itself. The deployed bundle does carry the endpoint and the namespace **[measured 2026-09-08, `app.c0c7162.js` fetched from the live site]**, so what is left is watching one fire.
